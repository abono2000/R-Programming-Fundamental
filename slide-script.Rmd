---
title: "R Programming Fundamental"
#author: 
#institute: "Data Science Nigeria (DSN)"
...
> The goal of this R workshop is to help you learn the most important skills in R that will allow you to do data science.
...
background-image: url(images/relax1.jpg)
...
.footer[
R code from throughout the slides is in the github repo [here](https://github.com/gbganalyst/R-Programming-Fundamental)
]
...
.w-100.lh-copy[
In the year `2020`, the R community celebrated the 20th year of R version `1.0.0` and the current version of R programming is `4.0.5` (Shake and Throw).
]
...
The current version of RStudio is 1.4.1106
...
Download R for Mac via this [link](https://cran.r-project.org/bin/macosx/R-4.0.5.pkg)
...
You can install RStudio via this [link](https://cran.r-project.org/) and choose the appropriate installer file for your operating system, download it, and then run it to install RStudio.
...
# Introduction to R
.w-100.lh-copy[
In this section, you will take your first steps with R. You will learn how to use the console as a calculator and how to assign variables. You will also
get to know the basic data types in R. 
]
...
.w-100.lh-copy[
R works with numerous atomic classes of objects. Some of the most basic
atomic data types are:

- Numeric such as integer (4, -2) or float (4.7)

- Character such as texts (or string) values e.g. "Nigeria", "Corona virus", "baby"
...
description) in R. You can then later use this variable’s name to easily access the value or the object that is stored with it. You can create a variable with the help of `<-` or `=` sign. 
]
...
.w-100.lh-copy[
In order to convert the data, you need to use the `as.datatype_name` function. As part of this function, you specify the data type you are converting to. The following table shows an example of the function:
]
  ...
  ~ `Data type converting to`,                     ~`How to do it`,
     ...
     "character", "as.character(variable_name)",
       "logical",   "as.logical(variable_name)",
        "factor",    "as.factor(variable_name)"
  ) %>% kable()
...
.w-100.lh-copy[
If R cannot convert something, it will return an `NA` value. This is its way of saying that it tried to convert the value, but it couldn't find anything that would logically match with the conversion. The following scenarios will display the `NA` result:
]

--

- Converting mixed-characters with numbers, e.g. `"334cm"`

--

- Converting any text to logical, except `T` or `F`
...
.w-100.lh-copy[
Vector is a one-dimensional array that can hold data type. In `R`, you can create a vector with the combine function `c()`. You place the vector elements separate by a comma between the parenthesis.
]
...
.w-100.lh-copy[
Let’s simulate `10` flips of a biased coin, where the probably of getting a Head (`H`) is `0.7` and the probability of a Tail(`T`) is `0.3`.
]
...
Draw 15 samples from a Poisson distribution with `lambda = 45`.
...
Draw 10 samples from a Normal distribution with `mean = 30`, `sd = 5`
...
.w-100.lh-copy[
Dataframe is another way to put data in a table! Unlike matrices, dataframe can have different types of data!

A dataframe has the variables of a data set as `columns` and the observations as `rows`. This will be a familiar concept for those coming from different statistical software packages such as `Excel`, `SPSS`, or `STATA`. We create a dataframe by using `data.frame()` function.
]
---
...
.w-100.lh-copy[
Create a dataframe with columns name `age`, `gender`, and `bank account`.
]
...
.w-100.lh-copy[
The number `1 2 3 4 5` at the left hand side on your `console` are `row` labels. Also note that each **column** in a dataframe is a **vector**!
]
...
.w-100.lh-copy[
Create a dataframe with columns `states`, `confirmed cases`, `recovered cases` and `death cases`.]
...
**4.** `names()`: prints each column name in a  dataframe. 

--

**5.** `nrow()`: returns the number of rows present in a `dataframe`.

--

**6.** `ncol()`: returns the number of columns present in a `dataframe`.

--

**7.** `dim()`: returns the number of rows and columns in a dataframe.

--

.w-100.lh-copy[
**8.** `View()`: invoke a spreadsheet-style data viewer on a dataframe. 
]

--

.w-100.lh-copy[
That is, it opens a new tab in the RStudio to display the entire dataframe in a spreadsheet-style.
]

--
.w-100.lh-copy[
**9.** `summary()`: returns summary or descriptive statistics of all columns in a dataframe
]
...
---
.w-100.lh-copy[
Every column in a dataframe has a name and if you can recall, we can print the names attribute of a dataframe (i.e. `iris`) by using:
]
...
.w-100.lh-copy[
and to access a specific column in a dataframe by name, you will use the `$` operator in the form of `df$colname`, where `df` is the name of the dataframe and `colname` is the name of the column you are interested in. This operation will then return the column you want as a vector.
...
Use the `$` operator to get a vector of `Sepal.Length` from the `iris` dataframe:
...
Use the `$` operator to get a vector of `Species` from the `iris` dataframe:
...
---
.w-100.lh-copy[
Because the `$` operator returns a vector, you can easily calculate descriptive statistics on columns of a dataframe by applying your favorite vector function (like `mean()`, `sd()`, or `table()`) to a column using `$`. 
]

--

.w-100.lh-copy[
Let’s calculate the mean of `Sepal.Length` with `mean()` and the frequency of each `Species` with `table()`in the iris dataframe:
]
...
## Slicing dataframe with [ , ]

---

.w-100.lh-copy[
Just like a matrix, you can access specific data in a dataframe by using square brackets `[ , ]`. To do this, we use the notation `data[rows, cols]`, where rows and columns are vectors of integers.
]
...
```{r, echo=FALSE}
dat <- tibble::tribble(
      ~`Dataframe slicing`,                        ~Interpretation,
...
---
.w-100.lh-copy[
R programming indicates any missing value in a dataframe with `NA` and to check whether a dataframe has a missing value in any of its columns, we use the command `anyNA()`.
]
...
.w-100.lh-copy[
To get the total number of missing values in a dataframe, we use the command `sum(is.na(dataframe))`
where `is.na(dataframe)` create a logical value `TRUE/FALSE` where there is a presence/absence of a missing value in each column of the dataframe.]
...
The `inspectdf::inspect_na(dataframe_name)` will help you get the number of missing values in each of the columns of the dataframe. Please make sure you install `inspectdf` package.
...
.w-100.lh-copy[
A list in **R** allows you to gather a variety of objects under one name (that is, the name of the list) in an ordered way. These objects can be `matrix`, `vector`, `dataframe`, even `other lists`. It is not even required that these objects are related to each other in any way.
...
.w-100.lh-copy[
This tutorial introduced you to `R` and `RStudio`. You also learnt various atomic data types and different data structure available in R programming.
]
