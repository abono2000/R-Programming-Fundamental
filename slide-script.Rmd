...
class(factor(c("Male", "Female", "Female", "Male", "Female"))
)
```
...
.w-100.lh-copy[
A vector is a one-dimensional array that can hold data type. In `R`, you can create a vector with the combine function `c()`. You place the vector elements separate by a comma between the parenthesis.
]
...
---
.w-100.lh-copy[
R programming indicates any missing value in a data frame with `NA` and to check whether a data frame has a missing value in any of its columns, we use the command `anyNA()`.
]
...
.w-100.lh-copy[
To get the total number of missing values in a data frame, we use the command `sum(is.na(dataframe))`
