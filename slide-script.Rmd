...
Repeat elements for a certain number of time
...
.w-100.lh-copy[
Let’s simulate `10` flips of a biased coin, were the probably of getting a Head (`H`) is `0.7` and the probability of a Tail(`T`) is `0.3`.
]
...
## Statistical distribution- Simulating data
...
```{r, eval=FALSE}
# Make a dataframe with columns named age and gender

demographic_data <- data.frame(age = c(16, 18, 13, 22, 18), gender = c("Female", "Female", "Male",   "Male",  "Female"),   
...
women_data <- data.frame(gender, height, weight) # The output is ?
...
View(data)
```
```{r, out.height= "70%", out.width = "90%", echo=FALSE}
...
`sex`, and the third is `logical` with name `pass`.
]
